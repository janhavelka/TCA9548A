...
/// Optional hard reset callback (RESET pin pulse)
...
struct Config {
  ...
  HardResetFn hardReset = nullptr;        ///< Optional hardware reset (RESET pin pulse)
  ...
  // === Device Settings ===
  uint8_t i2cAddress = 0x70;              ///< I2C address: 0x70–0x77
  ...
  // === Recovery ===
  uint32_t recoverBackoffMs = 100;        ///< Minimum ms between recover() attempts (0..600000)
